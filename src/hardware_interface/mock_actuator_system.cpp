...
::hardware_interface::CallbackReturn MockActuatorSystem::on_init(
  ...
  RCLCPP_INFO(rclcpp::get_logger("MockActuatorSystem"),
...
::hardware_interface::return_type MockActuatorSystem::read(
  ...
  // In a real system, this would read from hardware encoders/sensors.
  // For the mock, the states are already updated by write().
  // Nothing to do — states are directly accessible via pointers.
...
::hardware_interface::return_type MockActuatorSystem::write(
  ...
  //   acceleration = (effort_cmd - damping * velocity) / inertia
  ...
  for (std::size_t i = 0; i < hw_positions_.size(); ++i) {
    ...
    const double kp = 100.0;  // Position gain
    const double kd = 10.0;   // Derivative gain
