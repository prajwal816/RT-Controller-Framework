...
int main(int /*argc*/, char** /*argv*/) {
  ...
  std::printf("║  Cycles:     %5lu                                       ║\n",
              static_cast<unsigned long>(kNumCycles));
  std::printf("║  Duration:   %5.1f s                                     ║\n",
  ...
  // ─── Run the 1kHz loop ────────────────────────────────────────────────
  std::printf("[RT Loop] Starting %lu cycles at %u Hz...\n\n",
              static_cast<unsigned long>(kNumCycles), kFrequencyHz);
  ...
  for (uint64_t cycle = 0; cycle < kNumCycles; ++cycle) {
    ...
    if ((cycle + 1) % 1000 == 0) {
      ...
      std::printf("  [%5lus] Cycles: %lu | Mean Jitter: %.2f µs | "
                  "Max Jitter: %.2f µs | Mean Period: %.2f µs\n",
                  static_cast<unsigned long>((cycle + 1) / kFrequencyHz),
  ...
  std::printf("║  RESULT: %s                                          ║\n",
              pass ? "PASS ✓" : "FAIL ✗");
  std::printf("║  Max Jitter: %8.2f µs (threshold: 50.00 µs)          ║\n",
              stats.max_jitter_us);
  std::printf("╚═══════════════════════════════════════════════════════════╝\n");
