/**
 * @file test_rt_controller.cpp
 * @brief GTest unit tests for the RT controller plugin.
 *
 * Tests the controller lifecycle and verifies PD control behavior.
 * Note: Full integration tests require controller_manager; these are
 * unit-level tests of the controller's internal logic.
 */
...
TEST(RTControllerTest, JitterMonitorIntegration) {
  ...
  for (int i = 0; i < 50; ++i) {
    // Simulate ~1000 µs period with some jitter
